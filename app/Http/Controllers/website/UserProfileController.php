...
class UserProfileController extends Controller
{
   ...
   public function update(Request $request,$user)
   {
       ...
       /* if ($request->hasFile('image')) {
           ...
           $userIdAndFullName = $user->first_name . '_' . $user->last_name . '-' . $user->id; // making a directory name with the user's first_name, last_name & id
           $imagePath         = Storage::putFileAs("public/profiles/$userIdAndFullName", $imageRequest, $imageName);
    ...
    public function changePassword(string $username)
    {
        ...
        if ($user->id == auth()->user()->id) {
            ...
            return redirect()->route('profile.changePassword', auth()->user()->id);
        }
    }
// Update Password
...
{   
    ...
    try {
        ...
        return redirect()->route('profile.show', $user->id)->with('success', 'Your password has been updated successfully.');
